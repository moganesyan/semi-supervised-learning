...
from .alexnet_config import AlexNetConfig
...
class AlexNet(BaseModel):
    ...
    def __init__(self, model_config: AlexNetConfig) -> None:
    ...
    def _build_network(self) -> tf.keras.models.Model:
        """
            Build AlexNet architecture from the `model_config` object.
        ...
        for block_idx, block in enumerate(self._architecture):
            ...
            # handle max pooling blocks
            elif block['type'] == 'max_pool':
                x = layers.MaxPool2D(
                    pool_size = block['k'],
                    strides = block['s'],
                    padding = block['pad'],
